...
class TextCNN(object):
    ...
    def __init__(
            ...
            embedding_size, filter_sizes, num_filters, l2_reg_lambda=0.0, dense_dim=256):
        ...
        with tf.name_scope("dense"):
            w = tf.Variable(tf.truncated_normal([num_filters_total, dense_dim], stddev=0.1), name="dense_w")
            b = tf.Variable(tf.constant(0.1, shape=[dense_dim]), name="dense_b")
            wx_plus_b = tf.matmul(self.h_drop, w) + b
            self.dense = tf.nn.relu(wx_plus_b)

        with tf.name_scope("output"):
            W = tf.get_variable(
                "W",
                shape=[dense_dim, num_classes],
            ...
            self.scores = tf.nn.xw_plus_b(self.dense, W, b, name="scores")
